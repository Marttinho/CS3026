...
void copyFAT(fatentry_t *FAT)
{
   diskblock_t block;
   int index=0;
   for (int j = 1; j <= (MAXBLOCKS /(BLOCKSIZE / sizeof(fatentry_t))); j++)
   {
      for (int k = 0; k < (BLOCKSIZE / sizeof(fatentry_t)); k++)
      {
         block.fat[k]=FAT[index++];
      }
      writeblock(&block,j);
   }
}
...
void format(char *diskname)
{
   ...
   strncpy(block.data, diskname, sizeof(block.data));
   writeblock(&block, 0);
   ...
   {
      rootblock.dir.entrylist[i]=emptyEntry;
   }
